...
class TrafficGeneratorClient(QMainWindow):
    ...
    '''def edit_selected_stream(self):
        ...
        try:
            ...
            interface_item = self.stream_table.item(selected_row, 1)  # Assuming Interface is now column 1
            stream_name_item = self.stream_table.item(selected_row, 2)  # Assuming Name is now column 2

            # Validate that the necessary items are present
            if not interface_item or not stream_name_item:
                QMessageBox.warning(self, "Invalid Selection", "The selected row does not contain valid stream data.")
                return
            ...
            print(f"Selected Interface: {interface}, Selected Stream Name: {stream_name}")
        ...
        try:
            if interface not in self.streams:
                raise ValueError(f"Interface '{interface}' not found in streams dictionary.")

            stream = next(
                (s for s in self.streams[interface] if s.get("name") == stream_name),
                None
            )
            if not stream:
                raise ValueError(f"Stream '{stream_name}' not found in interface '{interface}'.")
        except Exception as e:
            QMessageBox.warning(self, "Error", str(e))
            return

        # Debugging: Log the stream data to be edited
        print(f"Stream edited: {stream}")
        ...
        try:
            ...
            if dialog.exec() == QDialog.Accepted:
                ...
                #QMessageBox.information(self, "Success", f"Stream '{stream_name}' has been updated successfully.")
            else:
                print("Edit operation cancelled.")  # Optional: Debugging information
        except Exception as e:
